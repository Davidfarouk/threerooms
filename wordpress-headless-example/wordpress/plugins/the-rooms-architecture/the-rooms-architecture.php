...
/**
 * Prevent duplicate titles for rental options
 */
function tr_prevent_duplicate_rental_options( $data, $postarr ) {
    // Only check for rental_option post type
    if ( $data['post_type'] !== 'rental_option' ) {
        return $data;
    }
    
    // Skip if post is being updated (not new)
    if ( ! empty( $postarr['ID'] ) ) {
        return $data;
    }
    
    // Check for existing post with same title
    $existing = get_page_by_title( $data['post_title'], OBJECT, 'rental_option' );
    if ( $existing ) {
        wp_die( 
            'Error: A rental option with the title "' . esc_html( $data['post_title'] ) . '" already exists. Please use a different title or edit the existing one.',
            'Duplicate Title',
...
add_filter( 'wp_insert_post_data', 'tr_prevent_duplicate_rental_options', 10, 2 );

/**
 * Add duplicate detection column to rental options admin
 */
function tr_rental_option_columns( $columns ) {
    $new_columns = array();
    $new_columns['cb'] = $columns['cb'];
    $new_columns['title'] = $columns['title'];
    $new_columns['duplicate_check'] = 'Duplicate Status';
    $new_columns['date'] = $columns['date'];
    return $new_columns;
}
add_filter( 'manage_rental_option_posts_columns', 'tr_rental_option_columns' );

function tr_rental_option_column_content( $column, $post_id ) {
    if ( $column === 'duplicate_check' ) {
        $post = get_post( $post_id );
        $title = $post->post_title;
        
        // Find other posts with same title
        ...
        if ( ! empty( $duplicates ) ) {
            ...
            echo '<span style="color: #d63638; font-weight: bold;">⚠️ ' . $count . ' duplicate' . ( $count > 1 ? 's' : '' ) . '</span>';
            echo '<br><small>Same title: "' . esc_html( $title ) . '"</small>';
        } else {
            echo '<span style="color: #00a32a;">✓ Unique</span>';
        }
    }
}
add_action( 'manage_rental_option_posts_custom_column', 'tr_rental_option_column_content', 10, 2 );

/**
 * Add bulk action to remove duplicates
 */
function tr_add_duplicate_cleanup_action( $bulk_actions ) {
    $bulk_actions['tr_remove_duplicates'] = 'Remove Duplicates (Keep First)';
    return $bulk_actions;
}
add_filter( 'bulk_actions-edit-rental_option', 'tr_add_duplicate_cleanup_action' );

function tr_handle_duplicate_cleanup( $redirect_to, $action, $post_ids ) {
    if ( $action !== 'tr_remove_duplicates' ) {
        return $redirect_to;
    }
    
    $removed = 0;
    $titles_processed = array();
    ...
    foreach ( $post_ids as $post_id ) {
        ...
        if ( in_array( $title, $titles_processed ) ) {
            continue;
        }
        
        // Find all posts with this title
        $duplicates = get_posts( array(
            'post_type' => 'rental_option',
            'post_status' => 'any',
            'title' => $title,
        ...
        $titles_processed[] = $title;
    }
    
    $redirect_to = add_query_arg( 'tr_duplicates_removed', $removed, $redirect_to );
    return $redirect_to;
}
add_filter( 'handle_bulk_actions-edit-rental_option', 'tr_handle_duplicate_cleanup', 10, 3 );

/**
 * Show admin notice after duplicate cleanup
 */
function tr_duplicate_cleanup_notice() {
    if ( ! empty( $_REQUEST['tr_duplicates_removed'] ) ) {
        $count = intval( $_REQUEST['tr_duplicates_removed'] );
        echo '<div class="notice notice-success is-dismissible"><p>';
        echo sprintf( 'Successfully removed %d duplicate rental option(s).', $count );
        echo '</p></div>';
    }
}
add_action( 'admin_notices', 'tr_duplicate_cleanup_notice' );
